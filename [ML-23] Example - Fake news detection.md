...
Q1. Clean the data, dropping the author (your model will not use this) and the articles with missing title or text.
...
Some very short titles may be expected to be deficient, but we don't find them so:
...
3. The embedding model suggested in the `sentence_transformers` website and many other sources) is `all-mpnet-base-v2`. This is the current version of a model called **MPNet** published in 2020 by a Microsoft research team. Try it with the title data. The embedding dimension is 768, so you can expect to improve the current results, and to face a slower encoding process. Also, the model would be downloaded to your computer the first time that you use it, as it happened with `all-MiniLM-L12-v2`.
